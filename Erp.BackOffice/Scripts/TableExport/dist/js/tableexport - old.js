        ...
        TableExport.prototype = {
            ...
            typeConfig: {
                ...
                date: {
                    ...
                    assert: function (v) {
                        var value_replace = v.replace(/\s|\d|[-\/]/g, ''); // xóa đi số và các kí tự -/, nếu như kết quà là chữ hoặc khoảng trắng thì không phải kiểu ngày
                        return !(/.*%/.test(v)) && !isNaN(Date.parse(v)) && (!isNaN(value_replace) || value_replace == '');
                    }
                }
            },
            exporters: {
            ...
            dateNum: function (v, date1904) {
                ...
                var result = (epoch - new Date(Date.UTC(1899, 11, 30))) / (24 * 60 * 60 * 1000); //1899, 11, 30, 1970, 1, 1
                return result.toFixed(0); //Math.floor(result);
            },
            /**
                ...
                for (var R = 0; R !== data.length; ++R) {
                    for (var C = 0; C !== data[R].length; ++C) {
                        ...
                        if (!cell.t) {
                            ...
                            else if (types.date.assert(cell.v)) cell.t = _TYPE.STRING; //_TYPE.DATE
