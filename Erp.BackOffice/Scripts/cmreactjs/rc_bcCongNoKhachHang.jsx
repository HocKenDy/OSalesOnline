...
function GetDatatable(url, title) {
    ...
    var BodyTable = React.createClass({
        btnClickTransactionLiabilities: function (event) {
            ...
            var start = $('#start').val();
            var end = $('#end').val();
            OpenPopup('/TransactionLiabilities/Report_LiabilitiesDetail/?StartDate=' + start + '&EndDate=' + end + '&TargetCode=' + event + '&SalerId=&strWarehouse=&TargetModule=Customer&IsPopup=true', 'Chi tiết công nợ', 0, 500);
            },
        btnClick: function (event) {
            ...
            OpenPopup('/Transaction/DanhSachMaHangChuaThanhToan/?CustomerCode=' + event + '&TargetType=Customer&IsPopup=true', 'Chi tiết công nợ', 0, 500);
        },
        render: function () {
