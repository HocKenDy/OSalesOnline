...
function GetDatatable(url, title) {
    ...
    function table(data) {
        ...
        var sumCenterInboundAmount = 0;
        var sumCenterOutboundQuantity = 0;
        var sumCenterOutboundAmount = 0;
        var sumLastRemain = 0;
        var sumLastAmount = 0;
            ...
            <table className="table table-bordered table-responsive" id="cTable">
                <thead>
                    ...
                    <tr>
                        ...
                        <th className="cell-center" style={{ width: 70 }}>Mã sản phẩm</th>
                        <th className="cell-center" style={{ width: 250 }}>Tên sản phẩm</th>
                        <th className="cell-center" style={{ width: 50 }}>Đơn vị</th>
                        <th className="cell-center" style={{ width: 60 }}>Đầu kỳ</th>
                        <th className="cell-center" style={{ width: 100 }}>Tổng tiền đầu kỳ</th>
                        <th className="cell-center" style={{ width: 70 }}>Nhập trong kỳ</th>
                        <th className="cell-center" style={{ width: 100 }}>Tổng tiền nhập trong kỳ</th>
                        <th className="cell-center" style={{ width: 70 }}>Xuất trong kỳ</th>
                        <th className="cell-center" style={{ width: 100 }}>Tổng tiền xuất trong kỳ</th>
                        <th className="cell-center" style={{ width: 70 }}>Cuối kỳ</th>
                        <th className="cell-center" style={{ width: 100 }}>Tổng tiền cuối kỳ</th>
                ...
                <tbody>
                    {
                        ...
                        data.map(function (obj, i) {
                            ...
                            sumFirstAmount += obj.First_Amount;
                            sumCenterInboundAmount += obj.Center_InboundAmount;
                            sumCenterOutboundAmount += obj.Center_OutboundAmount;
                            sumLastAmount += obj.Last_Amount;
                            return (
                                <tr key={i} className={name}>
                                    ...
                                    <td className="text-right">{ConvertSoAm(obj.First_Amount)}</td>
                                    <td className="text-right">{ConvertSoAm(obj.Center_InboundQuantity)}</td>
                                    <td className="text-right">{ConvertSoAm(obj.Center_InboundAmount)}</td>
                                    <td className="text-right">{ConvertSoAm(obj.Center_OutboundQuantity)}</td>
                                    <td className="text-right">{ConvertSoAm(obj.Center_OutboundAmount)}</td>
                                    <td className="text-right">{ConvertSoAm(obj.Last_Remain)}</td>
                                    <td className="text-right">{ConvertSoAm(obj.Last_Amount)}</td>
                                </tr>
                            );
                        })
                    }
                ...
                <tfoot>
                    <tr>
                        ...
                        <td className="text-right" style={{ fontWeight: 'bold' }}>{ConvertSoAm(sumFirstAmount)}</td>
                        <td className="text-right " style={{ fontWeight: 'bold' }} >{ConvertSoAm(sumCenterInboundQuantity)}</td>
                        <td className="text-right " style={{ fontWeight: 'bold' }} >{ConvertSoAm(sumCenterInboundAmount)}</td>
                        <td className="text-right " style={{ fontWeight: 'bold' }} >{ConvertSoAm(sumCenterOutboundQuantity)}</td>
                        <td className="text-right " style={{ fontWeight: 'bold' }} >{ConvertSoAm(sumCenterOutboundAmount)}</td>
                        <td className="text-right" style={{ fontWeight: 'bold' }} >{ConvertSoAm(sumLastRemain)}</td>
                        <td className="text-right" style={{ fontWeight: 'bold' }} >{ConvertSoAm(sumLastAmount)}</td>
                    </tr>
                </tfoot>
            </table>
        )
