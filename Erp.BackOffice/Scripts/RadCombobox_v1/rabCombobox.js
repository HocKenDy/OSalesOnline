...
(function ($) {
    ...
    function initComboBox($this, settings, rcb_id) {
        ...
        //    if ($(e.target).is(':not(.control-rcb, .rcbSlide)')) {
        //        $('.rcbSlide').removeClass('open').css({ "z-index": 6000, "display": "block", "height": "0px", "width": settings.width + "px", "overflow": "hidden" });
        //    }
        //});
    ...
    function initOrUpdateListOption(settings, $elemSelect, $list_option) {
        ...
        for (var iOpt = 0; iOpt < $list_option.length; iOpt++) {
            ...
            for (var numCol = 1; numCol <= settings.cols; numCol++) {
                ...
                row_item += '<td class="rcbCol colValue ' + (arr_col_class[numCol - 1] || "") + ' col' + numCol + class_col_hide + '" style="' + css_col_w + '">'
                    + value + '</td>';
            }
            row_item += '';
    ...
    function initEvent(settings, $elemSelect, $rcbSlide, rcb_id) {
        ...
        $rcbSlide.find('.rcb-input-search').keyup(function () {
            ...
            if (content_search != '') {
                ...
                var index = $(this).data("index");//$(this).closest('div').find('li').index($li_parent);

                $list_option.addClass('hideItem');
                for (var i = 0; i < $list_option.length; i++) {
                    var $col_find = $($list_option[i]).find('td').eq(index - 1);
                    ...
                    //    var content_col = $col_find.text().replace(/\D/g, '');
                    //    //console.log($col_find);
