...
namespace Erp.BackOffice.Account.Controllers
{
    ...
    public class AccountReportController : Controller
    {
        ...
        public ViewResult MenuReport()
        {
            var model = new List<PageMenuViewModel>();
            var pageReport = PageMenuRepository.GetPageMenus("vi-VN")
                .Where(item => item.IsVisible == true).ToList();
            AutoMapper.Mapper.Map(pageReport, model);
            return View(model);
        }
    }
}
