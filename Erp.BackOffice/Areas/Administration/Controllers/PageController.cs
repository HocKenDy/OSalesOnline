...
namespace Erp.BackOffice.Administration.Controllers
{
    ...
    public class PageController : Controller
    {
        ...
        public ActionResult IndexSubAction()
        {
            var q = _pageRepository.GetPages()
                    .Where(item => item.ControllerDisplayName.Contains("sub") && item.ParentActionId == null && item.IsVisible == true)
                    .OrderBy(item => item.ControllerDisplayName)
        ...
        public ActionResult SubActionDetail(int Id)
        {
            var q = _pageRepository.GetPages()
                    .Where(item => item.ParentActionId == Id)
                    .OrderBy(item => item.ControllerDisplayName)
                    .ThenBy(item => item.Name)
                    .ToList();
        ...
        public JsonResult EditInline(int Id, string fieldName, string value)
        {
            ...
            var flag = QueryHelper.UpdateFields("System_Page", field_value, Id);
        ...
        [HttpPost]
        public JsonResult EditGroupName(string oldValue, string newValue)
        {
            var list = _pageRepository.GetAllPage()
                .Where(item => item.GroupName == oldValue).ToList();
            foreach(var item in list)
            {
                item.GroupName = newValue;
                _pageRepository.UpdatePage(item);
            }
            return Json("success", JsonRequestBehavior.AllowGet);
        }

        [HttpPost]
        public JsonResult EditGroupNameByControllerName(string controllerName, string oldValue, string newValue)
        {
            var list = _pageRepository.GetAllPage()
                .Where(item => item.ControllerDisplayName == controllerName &&
                    ((string.IsNullOrEmpty(oldValue) && item.GroupName == null) || (!string.IsNullOrEmpty(oldValue) && item.GroupName == oldValue))).ToList();
            foreach (var item in list)
            {
                item.GroupName = newValue;
                _pageRepository.UpdatePage(item);
        ...
        [HttpPost]
        public JsonResult EditControllerDisplayName(string oldValue, string newValue)
        {
            var list = _pageRepository.GetAllPage()
                .Where(item => item.ControllerDisplayName == oldValue).ToList();
            foreach (var item in list)
            {
                item.ControllerDisplayName = newValue;
                _pageRepository.UpdatePage(item);
            }
            return Json("success", JsonRequestBehavior.AllowGet);
        }
        ...
        public ActionResult AddPageAjax(string AreaName, string ControllerName, string ActionName)
        {
            try
            {
                Page page = new Page { AreaName = AreaName, ControllerName = ControllerName, ControllerDisplayName = ControllerName, ActionName = ActionName, IsDelete = false, IsDeleted = false, Status = true, IsVisible = true };
