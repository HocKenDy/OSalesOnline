...
namespace Erp.BackOffice.Administration.Controllers
{
    ...
    public class UserController : Controller
    {
        ...
        public ActionResult Login(string returnUrl)
        {
            ...
            return View(new Erp.BackOffice.Models.LoginModel());
        }
        ...
        public ActionResult Login(LoginModel model, string returnUrl)
        {
            ...
            {
                user1 = _userRepository.GetByUserName(model.UserName);
            ...
            if (ModelState.IsValid && WebSecurity.Login(model.UserName, model.Password, persistCookie: model.RememberMe))
            {
                ...
                {
                    Helpers.Common.CurrentUser = null;
                }

                var sessionId = HttpContext.Session.SessionID;

                //HttpContext.ApplicationInstance.Application
                if (HttpContext.Application[user.UserName] != null)
                {
                    string sessionIdOther = Convert.ToString(HttpContext.Application[user.UserName]);
                    if (sessionIdOther != sessionId)
                    {
                        if (user.UserName != "demo" && user.UserName != "ketoan")
                        {
                            HttpContext.Session.Remove(sessionIdOther);
                        }
                        HttpContext.Application[user.UserName] = sessionId;
                    }
                    else
                    {
        ...
        public ViewResult Index(int? CategoryId, int? TypeId, int? StatusId, string txtSearch)
        {
            string a = "Admin".ToUpper();
            string h = "Host".ToUpper();

            var catId = Request.QueryString["CategoryId"] != null ? Convert.ToInt32(Request.QueryString["CategoryId"]) : 0;
            var status = Request.QueryString["StatusId"] != null ? Convert.ToInt32(Request.QueryString["StatusId"]) : 0;
            var type = Request.QueryString["TypeId"] != null ? Convert.ToInt32(Request.QueryString["TypeId"]) : 0;

            var model = _userRepository.GetUsersAvailable(CategoryId, TypeId, StatusId, txtSearch)
            ...
            if (ModelState.IsValid)
            {
                ...
                if (!CheckUsernameExists(model.UserName))
                {
                    ...
                    try
                    {
                        ...
                        //{
                        //    if (positionCode.ToUpper() != Globals.PositionCodeFSM.ToUpper())
                        //    {
                        //        //forum
                        //        try
                        //        {
                        //            SavinaForumRegisterService.RegisterServiceSoapClient client = new SavinaForumRegisterService.RegisterServiceSoapClient();
                        //            registryUserSucceed = client.CreateUser(model.UserName, model.Password, model.Email, model.LastName + " " + model.FirstName, model.DateOfBirth.GetHashCode() == 0 ? DateTime.Now : model.DateOfBirth.Value, model.Sex.Value ? 1 : 2);
                        //        }
                        //        catch (DbUpdateException)
                        //        {
                        //            TempData["AlertMessage"] = " Tạo User không thành công.";
                        //        }
                        //    }
                        //}
