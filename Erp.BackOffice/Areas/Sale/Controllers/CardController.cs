...
namespace Erp.BackOffice.Sale.Controllers
{
    ...
    public class CardController : Controller
    {
        ...
        {
            IEnumerable<ProductViewModel> q = ProductRepository.GetAllvwProduct().Where(x => x.Type == "card")
        ...
        public ActionResult Create(ProductViewModel model)
        {
            ...
            {
                var check_Code = ProductRepository.GetAllProductByType("card").Any(x => x.Code == model.Code);
                if (check_Code)
                {
                ...
                if (Request.Files["file-image"] != null)
                {
                    ...
                    {
                        string image_name = "card_" + Helpers.Common.ChuyenThanhKhongDau(Regex.Replace(Product.Code, @"\s+", "_")) + "." + file.FileName.Split('.').Last();
        ...
        public ActionResult CheckCodeExsist(int? id, string code)
        {
            ...
            var product = ProductRepository.GetAllProduct()
                .Where(item => item.Code == code && item.Type == "card").FirstOrDefault();
            if (product != null)
            {
        ...
        public ActionResult Edit(ProductViewModel model)
        {
            if (ModelState.IsValid)
            {
                if (Request["Submit"] == "Save")
                {
                    ...
                    var check_Code = ProductRepository.GetAllProductByType("card").Where(x => x.Code != Product.Code).Any(x => x.Code == model.Code);
                    if (check_Code)
                    {
                        ViewBag.FailedMessage = "Mã " + model.Code + " bị trùng!";
                        return View(model);
