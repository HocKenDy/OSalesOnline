...
namespace Erp.Domain.Sale.Repositories
{
    public class ProductInboundRepository : GenericRepository<ErpSaleDbContext, ProductInbound>, IProductInboundRepository
    {
        ...
        public ProductInbound GetProductInboundByCode(string Code)
        {
            return Context.ProductInbound.SingleOrDefault(item => item.Code == Code);
        }
        public vwProductInbound GetvwProductInboundByTransactionCode(string TransactionCode)
        {
            return Context.vwProductInbound.SingleOrDefault(item => item.Code == TransactionCode && (item.IsDeleted == null || item.IsDeleted == false));
        }
