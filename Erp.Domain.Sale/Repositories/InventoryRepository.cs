...
namespace Erp.Domain.Sale.Repositories
{
    public class InventoryRepository : GenericRepository<ErpSaleDbContext, Inventory>, IInventoryRepository
    {
        ...
        {
            return Context.vwInventory.Where(item => (item.IsDeleted == null || item.IsDeleted == false));
        }
        ...
        {
            return Context.vwInventory.Where(item => item.WarehouseId == Id && (item.IsDeleted == null || item.IsDeleted == false));
        }

        public IQueryable<vwInventory> GetAllvwInventoryByProductId(int Id)
        {
            return Context.vwInventory.Where(item => item.ProductId == Id && (item.IsDeleted == null || item.IsDeleted == false));
        }
        public IQueryable<vwInventory> GetAllvwInventoryByWarehouseIdAndProductManufacturer(int? Id, string Manufacturer)
        {
            return Context.vwInventory.Where(item => item.WarehouseId == Id && item.ProductManufacturer == Manufacturer && (item.IsDeleted == null || item.IsDeleted == false));
        }
        public IQueryable<vwInventory> GetAllvwInventoryByProductManufacturer(string Manufacturer)
        {
            return Context.vwInventory.Where(item => item.ProductManufacturer == Manufacturer && (item.IsDeleted == null || item.IsDeleted == false));
        }
